...
public sealed class OpenAIRerankerTests
{
    ...
    public async Task Should_ReturnNormalizedScore_When_ResponseIsSuccessful()
    {
        ...
        var sut = CreateSut(new FakeHttpMessageHandler(_ => JsonResponse(
    ...
    public async Task Should_PreserveInputOrder_When_ThereAreMultipleCandidates()
    {
        ...
        var sut = CreateSut(new FakeHttpMessageHandler(_ => JsonResponse(
            """{"choices":[{"message":{"content":"{\"score\": SCORE}"}}]}""".Replace("SCORE", scores.Dequeue(), StringComparison.Ordinal))));
    ...
    [Fact]
    public async Task Should_ThrowRerankingException_When_ResponseHasNoChoices()
    {
        var sut = CreateSut(new FakeHttpMessageHandler(_ => JsonResponse("""{"choices":[]}""")));

        await Should.ThrowAsync<RerankingException>(() => sut.RerankAsync("question", [CandidateFaker.Generate()]));
    }

    [Fact]
    public async Task Should_ThrowRerankingException_When_MessageContentIsNotAScorePayload()
    {
        var sut = CreateSut(new FakeHttpMessageHandler(_ => JsonResponse(
    ...
    private static OpenAIReranker CreateSut(FakeHttpMessageHandler handler, int candidatePoolSize = 25) =>
