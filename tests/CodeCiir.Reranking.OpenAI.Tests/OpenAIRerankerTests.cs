...
public sealed class OpenAIRerankerTests
{
    ...
    {
        var handler = new FakeHttpMessageHandler(_ => JsonResponse(
            """{"choices":[{"message":{"content":"{\"score\": 12345}"}}]}"""));
        var sut = CreateSut(handler);
    ...
    public async Task RerankAsync_MultipleCandidates_PreservesInputOrder()
    {
        ...
        var handler = new FakeHttpMessageHandler(_ => JsonResponse(
            """{"choices":[{"message":{"content":"{\"score\": SCORE}"}}]}""".Replace("SCORE", scores.Dequeue())));
        var sut = CreateSut(handler);
    ...
    public async Task RerankAsync_SendsModelAndQuestionAndCandidateText()
    {
        ...
        var sut = new OpenAIReranker(new HttpClient(handler) { BaseAddress = new Uri("http://fake/") }, "gpt-4o-mini", 25, 6);
        ...
        handler.LastRequestBody.ShouldContain("gpt-4o-mini");
    ...
    [Fact]
    public async Task RerankAsync_ResponseHasNoChoices_ThrowsRerankingException()
    {
        var handler = new FakeHttpMessageHandler(_ => JsonResponse("""{"choices":[]}"""));
        var sut = CreateSut(handler);
    ...
    [Fact]
    public async Task RerankAsync_MessageContentIsNotAScorePayload_ThrowsRerankingException()
    {
        var handler = new FakeHttpMessageHandler(_ => JsonResponse(
    ...
    {
        var sut = new OpenAIReranker(new HttpClient(new FakeHttpMessageHandler(_ => JsonResponse("{}"))) { BaseAddress = new Uri("http://fake/") }, "gpt-4o-mini", 42, 6);

        sut.CandidatePoolSize.ShouldBe(42);
    }

    private static OpenAIReranker CreateSut(FakeHttpMessageHandler handler) =>
        new(new HttpClient(handler) { BaseAddress = new Uri("http://fake/") }, "gpt-4o-mini", 25, 6);
