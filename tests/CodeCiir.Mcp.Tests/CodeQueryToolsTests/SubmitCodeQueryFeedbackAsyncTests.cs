...
public sealed class SubmitCodeQueryFeedbackAsyncTests : BaseCodeQueryToolsTests
{
    ...
    public async Task Should_PassParametersThroughAndMapTheStoredFeedback_When_ServiceSucceeds()
    {
        ...
        var result = await Sut.SubmitCodeQueryFeedbackAsync(projectId, question, true, [0.9], Agent);
    ...
    public async Task Should_ThrowMcpExceptionWithTheFailureMessage_When_ServiceFails()
    {
        ...
        FeedbackService.SubmitAsync(Arg.Any<Guid>(), Arg.Any<string>(), Arg.Any<bool>(), Arg.Any<IReadOnlyList<double>>(), Arg.Any<string?>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(Result<FeedbackResult>.FromFailure(failure));

        var exception = await Should.ThrowAsync<McpException>(() => Sut.SubmitCodeQueryFeedbackAsync(Guid.NewGuid(), Faker.Lorem.Sentence(), true, [], string.Empty));
