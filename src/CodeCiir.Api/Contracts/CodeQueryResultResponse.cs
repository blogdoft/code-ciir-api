...
/// Public raw-file URL for the symbol. This is the project's <c>gitRawUrl</c> concatenated
/// with the indexed source path; null when the project has no <c>gitRawUrl</c>.
/// </param>
...
/// match has no direct relations, or when <c>projectId</c> was omitted from the request (see
...
{
    public static CodeQueryResultResponse From(CodeQueryResult result) => new(
        result.Id,
        result.Kind,
        ...
        result.Similarity,
        result.RerankScore,
        (result.Relations ?? []).Select(CodeQueryRelationResponse.From).ToList());
}
