...
/// <summary>
/// Strongly typed binding of the "Keycloak" configuration section. Authentication is opt-in:
/// <see cref="FromConfiguration"/> returns <see langword="null"/> unless <see cref="Enabled"/> is
/// set, in which case the API stays fully open. Mirrors code-ciir-indexer's KeycloakOptions
/// (.specs/14-keycloak-auth.md), with one deliberate difference: the MCP endpoint ("/mcp") never
/// requires a token here, regardless of this section - see KeycloakAuthenticationExtensions.
/// </summary>
public sealed class KeycloakOptions
{
    ...
    /// <value><see langword="true"/> to require a Keycloak access token on REST endpoints; <see langword="false"/> (the default) leaves the API open and ignores the rest of the section.</value>
